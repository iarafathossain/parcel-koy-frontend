...
import CreateSpeed from "./create-speed";
import DeleteSpeed from "./delete-speed";
import EditSpeed from "./edit-speed";
import { speedColumns } from "./speed-columns";
import ViewSpeed from "./view-speed";

interface SpeedTableProps {
  initialQueryString: string;
}

type SpeedModalType = ModalType | "create";

const SpeedTable = ({ initialQueryString }: SpeedTableProps) => {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [isSortingTransitionPending, startSortingTransition] = useTransition();

  const [activeModal, setActiveModal] = useState<SpeedModalType>(null);
  const [selectedSpeed, setSelectedSpeed] = useState<ISpeed | null>(null);
  ...
  const speeds = speedsDataResponse?.data || [];
  const meta: PaginationMeta | undefined = speedsDataResponse?.meta;
  ...
  const handleView = (speed: ISpeed) => {
    setSelectedSpeed(speed);
    setActiveModal("view");
  };

  const handleEdit = (speed: ISpeed) => {
    setSelectedSpeed(speed);
    setActiveModal("edit");
  };

  const handleDelete = (speed: ISpeed) => {
    setSelectedSpeed(speed);
  ...
  const closeModal = () => {
    setActiveModal(null);
    setTimeout(() => setSelectedSpeed(null), 200);
  };
    ...
    <>
      <DataTable
        ...
        actions={{
          ...
          editLabel: "Edit Speed",
          deleteLabel: "Delete Speed",
        }}
        isLoading={isLoading || isFetching || isSortingTransitionPending}
        emptyMessage="No speeds found!"
        sorting={{
        ...
        search={{
          ...
          placeholder: "Search speed by name, slug...",
          debounceMs: 700,
      ...
      >
        {activeModal === "create" && <CreateSpeed onSuccess={closeModal} />}

        {activeModal === "view" && selectedSpeed && (
          <ViewSpeed speed={selectedSpeed} />
        )}

        {activeModal === "edit" && selectedSpeed && (
          <EditSpeed speed={selectedSpeed} onSuccess={closeModal} />
        )}

        {activeModal === "delete" && selectedSpeed && (
          <DeleteSpeed speed={selectedSpeed} onSuccess={closeModal} />
        )}
      </CommonModal>
    </>
  );
